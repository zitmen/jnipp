 ...
 *
 * This class is responsible for generating the C++ proxy forward declaration
 * header file for a specified Java class. The specifics of the code generation
 * process are guided by the settings supplied in the call to the
...
public class CPPProxyForwardHeaderGenerator {
    ...
    public void generate(ClassNode root, ProxyGenSettings proxyGenSettings)
        ...
        if (root.getPackageName().equals("") == true) {
            fullFileName = root.getCPPClassName() + "ProxyForward.h";
        } else {
            fullFileName = root.getPackageName().replace('.', File.separatorChar) + File.separatorChar + root.getCPPClassName() + "ProxyForward.h";
        }
